...
public class HandleInputMouse
{
    ...
    public void Process(bool cameraInputEnabled)
    {
        ...
        else if (mouse.leftButton.wasReleasedThisFrame == true)
        {
            ...
            bool blockedByUIOnRelease = IsPointerOverUIObject(mousePos);
            Debug.Log($"[InputDebug] MouseUp pos={mousePos} blockedByUIOnPress={m_inputBlockedByUI} blockedByUIOnRelease={blockedByUIOnRelease} isGalaxyView={m_camera.IsGalaxyView} cameraInputAllowed={cameraInputAllowed}");
            if (m_inputBlockedByUI == false && blockedByUIOnRelease == false)
            {
