...
public static class ExplorationEnemyFleetGenerator
{
    ...
    public static List<FleetInfo> GenerateWaves(ZoneConfig zoneConfig, int seed, int row, int col, DataTableShipPreset presetTable)
    {
        ...
        List<ShipPresetData> presets, ZoneConfig zoneConfig)
    {
        ...
        int costCap = ResolveCostCap(seed, row, col, fleetIndex, zoneConfig.enemyMaxCost, zoneConfig.enemyDeviation);
        ...
        while (remaining > 0 && fleetInfo.ships.Count < zoneConfig.enemyMaxShipsPerFleet - 1)
        {
            ...
            fleetInfo.ships.Add(new ShipInfo { shipPresetId = chosen.presetId });
        ...
        if (remaining > 0)
        {
            ...
            if (bestFit != null)
                fleetInfo.ships.Add(new ShipInfo { shipPresetId = bestFit.presetId });
        }

        if (fleetInfo.ships.Count == 0 && presets.Count > 0)
            fleetInfo.ships.Add(new ShipInfo { shipPresetId = presets[0].presetId }); // 폴백 — 구식 "grade=1" 폴백과 동일 의도
    ...
    private static int GetCost(List<ShipPresetData> presets, ShipInfo ship)
    {
        for (int i = 0; i < presets.Count; i++)
