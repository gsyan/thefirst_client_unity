Shader "SpaceFleet/Plastic"
{
    Properties
    {
        ...
        _SubsurfaceIntensity ("Subsurface Intensity", Range(0,1)) = 0.2
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float subsurface = pow(saturate(dot(viewDirWS, -mainLight.direction + normalWS)), 4) * _SubsurfaceIntensity;
                float3 subsurfaceColor = _SubsurfaceColor.rgb * subsurface * mainLight.color;

                // PBR lighting (plastic-like)
                float3 diffuse = albedo * mainLight.color * NdotL * mainLight.shadowAttenuation;

                float3 halfDir = normalize(mainLight.direction + viewDirWS);
                float NdotH = saturate(dot(normalWS, halfDir));
                float roughness = 1.0 - _Smoothness;
                float specularPower = exp2(8 * _Smoothness + 1); // Plastic has softer highlights
                ...
                // Ambient
                float3 ambient = SampleSH(normalWS) * albedo * 0.4; // Plastic has more ambient contribution

                // Highlight effect
                float3 highlight = _HighlightColor.rgb * _HighlightIntensity;

                float3 finalColor = diffuse + specular + ambient + emission + subsurfaceColor + highlight;
